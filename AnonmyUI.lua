-- AnonmyUI V1
-- Biblioteca de UI 100% nativa e autoral
...
-- Tabela de Tema para podermos alterar dinamicamente
local Theme = {
...
-- Mapeamento de elementos para atualizar cores dinamicamente
local UIReferences = {
    Accents = {},
    Backgrounds = {},
    Strokes = {}
...
function AnonmyUI:CreateWindow(config)
    ...
    table.insert(UIReferences.Backgrounds, {Obj = Main, Color = "Background"})
    
    local MainCorner = Create("UICorner", { CornerRadius = UDim.new(0, 8), Parent = Main })
    ...
    table.insert(UIReferences.Backgrounds, {Obj = Topbar, Color = "Topbar"})
    ...
    table.insert(UIReferences.Backgrounds, {Obj = TabContainer, Color = "Topbar"})
    ...
    function WindowAPI:CreateTab(name)
        ...
        function TabAPI:CreateToggle(text, default, callback)
            ...
            table.insert(UIReferences.Accents, Status)
        ...
        function TabAPI:CreateSlider(text, min, max, default, callback)
            ...
            table.insert(UIReferences.Accents, ValueLabel)
            ...
            table.insert(UIReferences.Accents, Fill)
    ...
    -- ==========================================
    local ConfigTab = WindowAPI:CreateTab("Configurações")
    ...
    ConfigTab:CreateToggle("Modo Neon (Borda Brilhante)", false, function(state)
        neonEnabled = state
        if state then
            MainStroke.Color = Theme.Accent
            MainStroke.Thickness = 2
    ...
    ConfigTab:CreateSlider("Cor Vermelha (R)", 0, 255, 0, function(val)
        currentRGB.R = val
        UpdateThemeColor()
    end)

    ConfigTab:CreateSlider("Cor Verde (G)", 0, 255, 200, function(val)
        currentRGB.G = val
        UpdateThemeColor()
    end)

    ConfigTab:CreateSlider("Cor Azul (B)", 0, 255, 200, function(val)
        currentRGB.B = val
        UpdateThemeColor()
    end)

    ConfigTab:CreateSlider("Transparência da Janela", 0, 1, 0, function(val)
    ...
    local keybindBtn = Create("TextButton", {
        Size = UDim2.new(1, -10, 0, 30),
        BackgroundColor3 = Theme.Element,
        Text = "Abrir/Fechar Menu: K",
        TextColor3 = Theme.Text,
        Font = Enum.Font.Gotham,
        TextSize = 12,
        Parent = ContentContainer:FindFirstChild("ScrollingFrame") -- Adiciona na aba de config
    })
    Create("UICorner", { CornerRadius = UDim.new(0, 4), Parent = keybindBtn })
    ...
    UserInputService.InputBegan:Connect(function(input, gameProcessed)
        ...
        if waitingForKey then
            ...
            keybindBtn.Text = "Abrir/Fechar Menu: " .. tostring(input.KeyCode.Name)
